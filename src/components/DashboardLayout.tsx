...
import { Search, Bell, Menu, User, Home, Settings, Users, FileText, PieChart, ChevronDown } from "lucide-react";
...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  const menuItems = [
    ...
    { name: "Tables", path: "/dashboard/tables", icon: FileText },
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto">
          <ul className="space-y-1">
            {menuItems.map((item) => {
              ...
              const isSettingsOrAccount = item.name === "Settings" || item.name === "My Account";
              
              return (
                <li key={item.name} className={isSettingsOrAccount ? "mt-auto" : ""}>
                  <NavLink
                    to={item.path}
                    className={cn(
        ...
        <div className="p-4 bg-orange-500 mt-auto">
          {/* Footer area */}
        </div>
      </aside>
